    ...
    [mgr setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status){
            case AFNetworkReachabilityStatusUnknown: // 未知网络
                [XJNetworking sharedNetworking].networkStats=StatusUnknown;
                break;
            case AFNetworkReachabilityStatusNotReachable: // 没有网络(断网)
                [XJNetworking sharedNetworking].networkStats=StatusNotReachable;
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN: // 手机自带网络
                [XJNetworking sharedNetworking].networkStats=StatusReachableViaWWAN;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi: // WIFI
                [XJNetworking sharedNetworking].networkStats=StatusReachableViaWiFi;
                break;
        }
    }];
    [mgr startMonitoring];
...
+ (instancetype)sharedNetworking{
    static XJNetworking *handler = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        handler = [[XJNetworking alloc] init];
    });
    return handler;
}
...
#pragma mark - 其他配置
- (void)requestSerializerConfig:(XJNetworkConfig *)request{
    self.requestSerializer =request.requestSerializer==XJRequestSerializerJSON ? [AFJSONRequestSerializer serializer]:[AFHTTPRequestSerializer serializer];
}

- (void)headersAndTimeConfig:(XJNetworkConfig *)request{
...
- (NSURLSessionDataTask *)uploadWithRequest:(XJNetworkConfig *)request
    ...
    NSURLSessionDataTask *uploadTask = [self POST:URLString parameters:request.parameters constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        [request.uploadDatas enumerateObjectsUsingBlock:^(XJUploadDataConfig *obj, NSUInteger idx, BOOL *stop) {
            if (obj.fileData) {
