...
@class XJUploadDataConfig;
@interface XJNetworkConfig : NSObject
/**
 *  用于标识不同类型的request
 */
@property (nonatomic,assign) XJRequestType requestType;

/**
 *  请求参数的类型
 */
@property (nonatomic,assign) XJRequestSerializer requestSerializer;

/**
 *  响应数据的类型
 */
@property (nonatomic,assign) XJResponseSerializer responseSerializer;
/**
 *  接口(请求地址)
 */
 ...
 *  为上传请求提供数据
 */
@property (nonatomic, strong, nullable) NSMutableArray<XJUploadDataConfig *> *uploadDatas;
/**
 *  用于维护 请求头的request对象
 */
...
/**
 上传文件数据的类
 */
@interface XJUploadDataConfig : NSObject
/**
 文件对应服务器上的字段
 */
